...
module switch_body(length, width, travel, taper = 1.1, wall_thickness = 1.4, sheath_wall_thickness = 1.2,
                   sheath_length = 3, cover_overhang = 1, cover_thickness = 0.6, plate_thickness = 1.5,
                   plate_tolerance = 0.2, stem_diameter = 4.1, sheath_tolerance = 0.2, lip_height = 1,
                   top_magnet_height = 2, top_magnet_diameter = 6, top_magnet_cover_thickness = -0.5,
                   magnet_height = 1.5, magnet_wall_thickness = 0.5, magnet_tolerance = 0.15, magnet_void = 0.0,
                   sheath_end_stop_thickness = 0.8, corner_radius = 0.5, droop_extra = 0.2, sheath_clip_width = 1.5,
                   top_clip_hole_length = 2.15, top_clip_hole_width = 3.15, stem_tolerance = 0.15,
                   bridge_thickness = 0.3, sheath_snug_magnet = true, sheath_inside_body = false, height = 0)
{
    ...
    sheath_depth = (cover_thickness + sheath_length + total_travel - top_magnet_cover_thickness + magnet_height +
                    magnet_wall_thickness * 2 + magnet_tolerance + magnet_void +
                    sheath_end_stop_thickness); // NOTE: Does not include the lip_height on purpose
    ...
    difference()
    {
        union()
        {
            ...
            difference()
            {
                ...
                //                adjusted_magnet_wall_thickness = sheath_inside_body ? 0 : magnet_wall_thickness;
                translate([
                    -length / 2.85, -width / 2.85,
                    body_height / 2 + top_magnet_height + magnet_wall_thickness + droop_extra -
                        top_magnet_cover_thickness +
                    sheath_length
                ...
                difference()
                {
                    ...
                    translate([
                        -length / 2 - top_magnet_diameter / 2.2, -width / 2 - top_magnet_diameter / 2.2,
                        -magnet_wall_thickness / 2 + top_magnet_height + magnet_wall_thickness + droop_extra -
                            top_magnet_cover_thickness +
                        sheath_length
                    ]) rotate([ 0, 0, 45 ])
                        cube(
                ...
                translate([
                    -length / 2 + top_magnet_diameter / 1.65, -width / 2 + top_magnet_diameter / 1.65,
                    -(magnet_wall_thickness + droop_extra) / 2 + top_magnet_height + magnet_wall_thickness +
                        droop_extra - top_magnet_cover_thickness +
                    sheath_length
                ]) difference()
                {
                    cube([ 2, 2, magnet_wall_thickness + droop_extra + 0.01 ], center = true);
                    translate([ 0.65, 0.65, 0 ]) rotate([ 0, 0, 45 ])
                ...
                // Cut out the sides near where the stem presses against the magnet so it doesn't get caught on the
                // sides
                translate([ -4.7, -1, top_magnet_height ]) cube([ 2, 1.6, top_magnet_height * 2 ], center = true);
                translate([ -1, -4.7, top_magnet_height ]) cube([ 1.6, 2, top_magnet_height * 2 ], center = true);
                if (cover_overhang)
                {
        ...
        // NOTE: center_adjust matches how high off the floor the Cherry cross (+) is in stem.scad.  This allows us to
        // accurately position the sheath so that the stem sits dead center.
        rotate([ 0, 0, 45 ]) translate([ -(sheath_height + sheath_tolerance * 2) / 2 + center_adjust, 0, 0 ])
            difference()
        {
            cube([ sheath_height + sheath_tolerance * 2, sheath_width + sheath_tolerance * 2, body_height ],
                 center = true);
            translate([ sheath_height / 2, 0, 0 ])
                cube([ cover_thickness, sheath_clip_width - sheath_tolerance * 2, body_height * 2 ], center = true);
        }
        ...
        if (!sheath_snug_magnet)
        {
            ...
            {
                cylinder(d = top_magnet_diameter + magnet_tolerance, // For just manget
                         h = top_magnet_height * 2 + magnet_tolerance + droop_extra, center = true);
            }
        }
        else
        {
            ...
            ])
            {
                rotate([ 0, 0, 22.5 ])
                    cylinder(d = top_magnet_diameter + magnet_wall_thickness * 3 +
                                 sheath_tolerance * 2, // For magnet + sheath holding it
                             h = top_magnet_height * 2 + magnet_tolerance + droop_extra + sheath_length, center = true,
                             $fn = fn);
            }
        }
        ...
        translate([
            ...
            //          -stem_diameter/4-top_magnet_diameter/1.75,
            -stem_diameter / 2 - (top_magnet_diameter - magnet_wall_thickness * 3) / 2.9,
            -stem_diameter / 2 - (top_magnet_diameter - magnet_wall_thickness * 3) / 2.9, top_magnet_height / 2 -
            top_magnet_cover_thickness
        ])
        {
            % cylinder(d = top_magnet_diameter + magnet_tolerance, h = top_magnet_height, center = true);
        }
