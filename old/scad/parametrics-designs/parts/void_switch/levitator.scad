...
/* NOTES:
    ...
    * The default configuration works with 4x1 through 4x2mm magnets (anything shorter than 2mm will end up stuck in
   there just fine--don't worry about it).
    * The default configuration will "eat" 0.4mm of your stem's cross height.  So keycaps won't be able to go all the
   way down (if you didn't print that part of the stem slightly longer to make up for it).  In testing, losing 0.4mm of
   height on the cross part of the stem makes absolutely no difference whatsoever.  The keycap still stays on just fine.
...
// Configurables
LEVITATOR_THICKNESS =
    0.4; // How thick the floor will be/how much of the cross part of the stem (+) gets eaten by THE LEVITATOR
...
levitator(MAGNET_DIAMETER, MAGNET_HEIGHT, magnet_distance = MAGNET_DISTANCE, magnet_tolerance = MAGNET_TOLERANCE,
          wall_thickness = WALL_THICKNESS, thickness = LEVITATOR_THICKNESS, stem_tolerance = STEM_TOLERANCE);
// levitator_double_sided(MAGNET_DIAMETER, MAGNET_HEIGHT, magnet_distance=MAGNET_DISTANCE,
// magnet_tolerance=MAGNET_TOLERANCE, wall_thickness=WALL_THICKNESS, thickness=LEVITATOR_THICKNESS,
...
BACKFORCE_WIDTH =
    11; // Needs to fit inside/under the keycap but not terribly tight--just enough to prevent it from rotating
// uplifting_jigaroo(BACKFORCE_LENGTH, BACKFORCE_WIDTH, stem_diameter=CHERRY_CYLINDER_DIAMETER, magnet_diameter=4,
// magnet_height=2, magnet_tolerance=0, wall_thickness=1.2);

module levitator(magnet_diameter, magnet_height, magnet_distance = 3.75, magnet_tolerance = MAGNET_TOLERANCE,
                 wall_thickness = 0.9, thickness = 1, stem_tolerance = 0.15)
{
    ...
    difference()
    {
        ...
        translate([ 0, 0, height / 2 + thickness + wall_thickness / 2 ]) // Stem cutout
            cube(
                [
                    CHERRY_CYLINDER_DIAMETER + stem_tolerance * 2, CHERRY_CYLINDER_DIAMETER + stem_tolerance * 2,
                    height +
                    wall_thickness
                ],
                center = true);
        ...
        // Make an angled hole in the top that stops the magnet from coming out while also letting you push it out if
        // needed to flip it around
        ...
        translate([ magnet_distance * 1.5, magnet_distance * 1.5, magnet_diameter / 1.35 ]) rotate([ 0, -50, 45 ])
            cube([ magnet_diameter, magnet_diameter * 2, magnet_diameter * 2 ], center = true);
    }
}

module levitator_double_sided(magnet_diameter, magnet_height, magnet_distance = 3.75,
                              magnet_tolerance = MAGNET_TOLERANCE, wall_thickness = 0.9, thickness = 1,
                              stem_tolerance = 0.15)
{
    // NOTE: stem_tolerance is the keycap stem; not the switch stem
    height = 2 + wall_thickness; // Just enough to grab the stem
    difference()
    {
        ...
        translate([ magnet_distance, magnet_distance, wall_thickness / 2 + (magnet_height + magnet_tolerance) - 0.01 ])
            cylinder(d1 = magnet_diameter, d2 = magnet_diameter / 1.5, h = wall_thickness, center = true);
        translate(
            [ -magnet_distance, -magnet_distance, wall_thickness / 2 + (magnet_height + magnet_tolerance) - 0.01 ])
            cylinder(d1 = magnet_diameter, d2 = magnet_diameter / 1.5, h = wall_thickness, center = true);
        // Cut off a few bits here and there so it can fit under more keycaps
        translate([ magnet_distance * 1.5, magnet_distance * 1.5, magnet_diameter / 1.35 ]) rotate([ 0, -50, 45 ])
            cube([ magnet_diameter, magnet_diameter * 2, magnet_diameter * 2 ], center = true);
...
// bottom and provide a backwards (upwards) force to ease the burden of bringing a heavy keycap back up.  In other
// words, it's for (heavy) spacebars.  NOTE: The *length* is really just the distance between the center of the
// magnets--not the actual length of the thing. NOTE: Requires matching magnets (same locations underneath) in the
// keyboard plate!
module uplifting_jigaroo(length, width, stem_diameter = CHERRY_CYLINDER_DIAMETER, magnet_diameter = 4,
                         magnet_height = 2, magnet_tolerance = 0, wall_thickness = 1.2)
{
